...
class Peluquero {
  ...
  factory Peluquero.fromMap(Map<String, dynamic> json) => Peluquero(
        ...
        horario: Map.from(json["horario"])
            .map((k, v) => MapEntry<String, bool>(k, v)),
        imagen: json["imagen"],
        nombre: json["nombre"],
        servicios: Map.from(json["servicios"])
            .map((k, v) => MapEntry<String, Servicio>(k, Servicio.fromMap(v))),
  ...
  Map<String, dynamic> toMap() => {
        "atiende": atiende,
        "horario":
            Map.from(horario).map((k, v) => MapEntry<String, dynamic>(k, v)),
        "imagen": imagen,
        "nombre": nombre,
        "servicios": Map.from(servicios)
